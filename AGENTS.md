...
- 开发结果必须 Git 提交并推送到此仓库；用户已授权单 Agent 串行开发：本地必要测试通过后直接推送 main，不要求 PR/MR 或 GitHub API 权限；交付包含 main 提交链接。推送前 fetch，必要时正常合并最新 main 并复测，不强推、不覆盖他人提交；远端保护若拒绝则报告实际限制，不自行取消保护。下载附件可以补充交付，不能替代 Git 上传。
- 桌面键鼠单机副本，沿用原生 HTML/CSS/Canvas。保持离线直接打开能力，不擅自引入后端、账号或付费服务。
- 页面模板为 `index.html.in`、`style.css`、`game.js`；生成背景位于 `assets/`。修改后执行 `npm run build`，同步提交生成的 `index.html`。
